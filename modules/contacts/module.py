...
class ContactsModule(ModuleBase):
    ...
    def on_load(self):
        """Initialise CNContactStore et demande les permissions d'accès aux contacts."""
        try:
            ...
            step_error("contacts", "pyobjc-framework-Contacts not installed — run: pip install pyobjc-framework-Contacts")
            self.store = None
    ...
    def _soundex_token(token: str) -> str:
        """Soundex sur un token normalisé (sans accents)."""
        try:
    ...
    def _score(query: str, candidate: str) -> float:
        """
        Score de correspondance entre la requête et un nom de contact.
        Combine trois niveaux :
          1. Correspondance textuelle exacte / partielle
          2. Jaccard sur les tokens
          3. Correspondance phonétique Soundex (robuste aux erreurs STT)
    ...
    def _find_best_matches(self, query: str, top_k: int = 3, threshold: float = 0.4) -> List[Any]:
        """
        Charge tous les contacts et retourne les top_k les plus proches
        de la requête selon le score de correspondance.
        """
        keys = [
    ...
    def _format_contact(contact) -> Dict[str, Any]:
        """Convertit un CNContact en dict lisible."""
        full_name = f"{contact.givenName()} {contact.familyName()}".strip()
        org = str(contact.organizationName() or "")
